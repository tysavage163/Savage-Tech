...
global.antiForwardConfig = {};
global.antiForwardWarnings = {};
...
└─────────────────────────┘`;
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        if (from && from.endsWith('@g.us') && !isMe && msg.message) {
            const isForwarded = msg.message?.extendedTextMessage?.contextInfo?.isForwarded === true ||
                                msg.message?.conversation?.includes?.(false) ||
                                false;
            if (isForwarded) {
                const cfg = global.antiForwardConfig?.[from];
                if (cfg && cfg.enabled) {
                    let isSenderAdmin = false;
                    try {
                        const meta = await sock.groupMetadata(from);
                    ...
                    let shouldDelete = (action === 'delete' || action === 'warn' || action === 'kick');
                    let shouldWarn = (action === 'warn');
                    let shouldKick = (action === 'kick');
                    if (shouldDelete) {
                        try { await sock.sendMessage(from, { delete: msg.key }); } catch (err) {}
                    }
                    if (shouldWarn) {
                        if (!global.antiForwardWarnings[from]) global.antiForwardWarnings[from] = {};
                        const warns = (global.antiForwardWarnings[from][sender] || 0) + 1;
                        global.antiForwardWarnings[from][sender] = warns;
                        await sock.sendMessage(from, { text: `⚠️ @${sender.split('@')[0]}, forwarding is not allowed. Warning ${warns}/${warnLimit}`, mentions: [sender] });
                        if (warns >= warnLimit) {
                            try {
                                await sock.groupParticipantsUpdate(from, [sender], 'remove');
                                delete global.antiForwardWarnings[from][sender];
                                await sock.sendMessage(from, { text: `🚫 @${sender.split('@')[0]} removed for repeated forwarding.`, mentions: [sender] });
                            } catch (err) {}
                        }
                    } else if (shouldKick) {
                        try {
                            await sock.groupParticipantsUpdate(from, [sender], 'remove');
                            await sock.sendMessage(from, { text: `🚫 @${sender.split('@')[0]} removed (forwarding not allowed).`, mentions: [sender] });
                        } catch (err) {}
                    }
                    return;
                }
