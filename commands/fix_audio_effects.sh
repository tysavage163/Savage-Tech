...
module.exports = {
  ...
  async execute(sock, msg, args) {
    ...
    if (!url.match(/\.(mp3|wav|ogg|m4a)(\?|$)/i)) {
      await sock.sendMessage(msg.key.remoteJid, { text: '⚠️ This command only works with direct audio file URLs (ending in .mp3, .wav, etc.).\nIt does NOT work with Spotify links, YouTube links, or web pages.' });
      return;
    }
...
ENDJS
  ...
  echo "✅ Created ${cmd}.js (requires direct audio URL)"
done
echo "Audio effects regenerated."
