...
module.exports = {
  name: 'dagd',
  category: 'tools',
  description: 'Shorten URL with da.gd',
  async execute(sock, msg, args) {
    const longUrl = args[0];
    if (!longUrl || !longUrl.startsWith('http')) {
      return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .dagd <https://example.com/long/url>' });
    }
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mention = [senderJid];
    try {
      const apiUrl = `https://apis.xwolf.space/api/short/dagd?url=${encodeURIComponent(longUrl)}`;
