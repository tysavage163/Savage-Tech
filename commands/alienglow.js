...
module.exports = {
  name: 'alienglow',
  category: 'Ephoto',
  description: 'Generate alien-glow text effect',
  async execute(sock, msg, args) {
    const text = args.join(' ');
    if (!text) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .alienglow <text>' });
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mentions = [senderJid];
    try {
      const apiUrl = `https://apis.xwolf.space/api/textpro/alien-glow?text=${encodeURIComponent(text)}`;
