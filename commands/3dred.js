...
module.exports = {
  name: '3dred',
  category: 'Ephoto',
  description: 'Generate 3d-red text effect',
  async execute(sock, msg, args) {
    const text = args.join(' ');
    if (!text) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .3dred <text>' });
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mentions = [senderJid];
    try {
      const apiUrl = `https://apis.xwolf.space/api/textpro/3d-red?text=${encodeURIComponent(text)}`;
      ...
      const caption = `🎨 *Text Effect: 3dred*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
