...
module.exports = {
  name: 'tiktokaudio',
  category: 'download',
  description: 'Download from tiktokaudio',
  async execute(sock, msg, args) {
    const url = args[0];
    if (!url) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .tiktokaudio <URL>' });
    ...
    try {
      const apiUrl = `https://apis.xwolf.space/download/tiktok/audio?url=${encodeURIComponent(url)}`;
      ...
      const isVideo = 'tiktokaudio'.includes('video') || 'tiktokaudio'.includes('mp4') || 'tiktokaudio' === 'tiktok' || 'tiktokaudio' === 'instagram' || 'tiktokaudio' === 'facebook' || 'tiktokaudio' === 'twitter' || 'tiktokaudio' === 'snapchat';
      const isAudio = 'tiktokaudio'.includes('mp3') || 'tiktokaudio'.includes('audio');
      const isText = 'media' === 'text';

      if (isText) {
        // Send as text (info or search results)
        let text = `📁 *Download Info (tiktokaudio)*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE\n\n`;
      ...
      const caption = `📥 *Download: tiktokaudio*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
      if (isVideo) {
