...
module.exports = {
    name: 'video',
    category: 'download',
    description: 'Download HD video from YouTube',
    async execute(sock, msg, args) {
        const from = msg.key.remoteJid;
        const query = args.join(' ');
        if (!query) {
            return sock.sendMessage(from, { text: '🎥 Usage: .video <song name or YouTube URL>' }, { quoted: msg });
        }
        ...
        try {
            ...
            const endpoint = `https://apis.xwolf.space/download/hd?url=${encodeURIComponent(videoUrl)}`;
            ...
            await sock.sendMessage(from, { text: '❌ Failed to download video. Try another name or URL.' }, { quoted: msg });
        }
    }
};
