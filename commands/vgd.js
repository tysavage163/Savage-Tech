...
module.exports = {
  ...
  async execute(sock, msg, args) {
    ...
    if (!longUrl || !longUrl.startsWith('http')) {
      return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .vgd <https://example.com/long/url>' });
    }
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mention = [senderJid];
    try {
      const apiUrl = `https://apis.xwolf.space/api/short/vgd?url=${encodeURIComponent(longUrl)}`;
