...
module.exports = {
  name: '3dgreen',
  category: 'Ephoto',
  description: 'Generate 3d-green text effect',
  async execute(sock, msg, args) {
    const text = args.join(' ');
    if (!text) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .3dgreen <text>' });
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mentions = [senderJid];
    try {
      const apiUrl = `https://apis.xwolf.space/api/textpro/3d-green?text=${encodeURIComponent(text)}`;
      ...
      const caption = `🎨 *Text Effect: 3dgreen*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
