...
module.exports = {
  ...
  description: 'Apply 8d effect to audio',
  async execute(sock, msg, args) {
    const url = args[0];
    if (!url) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .8d <audio_url>' });
    if (!url.startsWith('http')) return sock.sendMessage(msg.key.remoteJid, { text: '❌ Invalid URL.' });
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mention = [senderJid];
    try {
      await sock.sendMessage(msg.key.remoteJid, { text: '🎧 Applying 8d effect for @' + senderName + '...', mentions: mention });
      const apiUrl = 'https://apis.xwolf.space/api/audio/8d?url=' + encodeURIComponent(url);
      ...
      const caption = '✨ 8d Effect Applied\n👤 REQUESTED BY: @' + senderName + '\n🚀 POWERED BY SAVAGE-CORE';
      await sock.sendMessage(msg.key.remoteJid, { audio: audioBuffer, mimetype: 'audio/mpeg', fileName: '8d_effect.mp3', caption: caption, mentions: mention });
    } catch (err) {
      console.error('8d error:', err);
