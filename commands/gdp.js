...
module.exports = {
  ...
  async execute(sock, msg, args) {
    const param = args[0];
    if (!param && 'gdp' !== 'market' && 'gdp' !== 'gold' && 'gdp' !== 'news') {
      return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .gdp <parameter>' });
    }
    ...
    try {
      let apiUrl = `https://apis.xwolf.space/api/economy/gdp`;
      let paramLabel = '';
      ...
      if ('gdp' === 'forex') {
        ...
        // market, gold, news
        apiUrl = `https://apis.xwolf.space/api/economy/gdp`;
      }
