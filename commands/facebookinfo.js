...
module.exports = {
  name: 'facebookinfo',
  category: 'download',
  description: 'Download from facebookinfo',
  async execute(sock, msg, args) {
    const url = args[0];
    if (!url) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .facebookinfo <URL>' });
    ...
    try {
      const apiUrl = `https://apis.xwolf.space/download/facebook/info?url=${encodeURIComponent(url)}`;
      ...
      const isVideo = 'facebookinfo'.includes('video') || 'facebookinfo'.includes('mp4') || 'facebookinfo' === 'tiktok' || 'facebookinfo' === 'instagram' || 'facebookinfo' === 'facebook' || 'facebookinfo' === 'twitter' || 'facebookinfo' === 'snapchat';
      const isAudio = 'facebookinfo'.includes('mp3') || 'facebookinfo'.includes('audio');
      const isText = 'text' === 'text';

      if (isText) {
        // Send as text (info or search results)
        let text = `📁 *Download Info (facebookinfo)*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE\n\n`;
      ...
      const caption = `📥 *Download: facebookinfo*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
      if (isVideo) {
      ...
      console.error('facebookinfo error:', err);
