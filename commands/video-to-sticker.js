...
module.exports = {
  ...
  category: 'tools',
  description: 'Convert media using video-to-sticker (reply to image/video/sticker OR provide URL)',
  async execute(sock, msg, args) {
    ...
    if (!mediaBuffer && !providedUrl) {
      return sock.sendMessage(msg.key.remoteJid, { 
        text: '❓ Usage: .video-to-sticker\n   - Reply to an image/video/sticker\n   - Or provide a direct URL: .video-to-sticker https://example.com/media.jpg',
        mentions: [jid]
      });
    }
    ...
    try {
      ...
      if (mediaBuffer) {
        ...
        apiUrl = `https://apis.xwolf.space/api/converter/video-to-sticker?url=${encodeURIComponent(dataUrl)}`;
      } else {
        apiUrl = `https://apis.xwolf.space/api/converter/video-to-sticker?url=${encodeURIComponent(providedUrl)}`;
      }
      ...
      const caption = `✅ *Converted via video-to-sticker*\n👤 REQUESTED BY: @${sender}\n🚀 POWERED BY SAVAGE-CORE`;
