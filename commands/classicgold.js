...
module.exports = {
  name: 'classicgold',
  category: 'Ephoto',
  description: 'Generate classic-gold text effect',
  async execute(sock, msg, args) {
    const text = args.join(' ');
    if (!text) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .classicgold <text>' });
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mentions = [senderJid];
    try {
      const apiUrl = `https://apis.xwolf.space/api/textpro/classic-gold?text=${encodeURIComponent(text)}`;
