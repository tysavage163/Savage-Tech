...
async function startSavage() {
    ...
    sock.ev.on("connection.update", async (update) => {
        ...
        if (connection === "open") {
            ...
            try {
                ...
                try {
                    ...
                    if (typeof sock.newsletterFollow === "function") {
                        ...
                        console.log("✅ Auto-followed support channel.");
                    } else {
                        console.log("⚠️ newsletterFollow() not supported in this Baileys version.");
                    }

                } catch (e) {
                    console.log("⚠️ Channel auto-follow failed:", e.message);
                }

                // ===== AUTO JOIN SUPPORT GROUP =====
                try {
                    const inviteCode = SUPPORT_GROUP_LINK
                        .split("/")
                        .pop()
                        .split("?")[0];
                    ...
                    console.log("✅ Joined support group automatically.");

                } catch (e) {
                    console.log("⚠️ Group auto-join failed:", e.message);
                }

            } catch (err) {
                console.log("⚠️ Auto-support system error:", err.message);
            }
            ...
            const randomQuote =
                startQuotes[Math.floor(Math.random() * startQuotes.length)];
...
✅ Auto-connected to support systems.

📢 Anti-delete is active.
Deleted messages will be forwarded here.
...
🚀 STATUS: ONLINE`;
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        if (from && from.endsWith('@g.us')) {
            ...
            global.messageCounts[from][sender] =
                (global.messageCounts[from][sender] || 0) + 1;
        ...
        if (from && from.endsWith('@g.us')) {
            ...
            if (antiLinkEnabled) {
                ...
                const urlPattern =
                    /(https?:\/\/[^\s]+|www\.[^\s]+|\.[a-z]{2,}\/[^\s]*|chat\.whatsapp\.com\/[A-Za-z0-9]+)/i;

                if (urlPattern.test(rawText)) {
                    if (!global.violationWarnings[from]) {
                        global.violationWarnings[from] = {};
                    }

                    const currentWarnings =
                        global.violationWarnings[from][senderJid] || 0;
                    ...
                    if (newWarningCount < 3) {
                        const randomQuote =
                            warnQuotes[Math.floor(Math.random() * warnQuotes.length)];

                        const warningText =
`⚠️ *VIOLATION* @${senderJid.split('@')[0]}
...
┕━━━━━━━━━━━━━━━╼`;
                        ...
                        const kickQuote =
                            finalKickQuotes[Math.floor(Math.random() * finalKickQuotes.length)];

                        const kickMessage =
`⚠️ *AUTOMATIC KICK* @${senderJid.split('@')[0]}
...
┕━━━━━━━━━━━━━━━╼`;
                        ...
                        try {
                            ...
                            await sock.sendMessage(from, {
                                text: `❌ Could not kick user. Make sure I am an admin.`
                            });
                        }
    ...
    sock.ev.on("messages.update", async (updates) => {
        ...
        for (const update of updates) {
            ...
            if (deletedMsg.conversation) {
                content = deletedMsg.conversation;
            } else if (deletedMsg.imageMessage?.caption) {
                content = `${deletedMsg.imageMessage.caption} (image)`;
            } else if (deletedMsg.videoMessage?.caption) {
                content = `${deletedMsg.videoMessage.caption} (video)`;
            } else if (deletedMsg.extendedTextMessage?.text) {
                content = deletedMsg.extendedTextMessage.text;
            ...
            if (eventHandler && typeof eventHandler.sendWelcome === 'function') {
                ...
                for (let participant of participants) {
                    if (action === 'add' && global.welcomeEnabled[id] === true) {
                        await eventHandler.sendWelcome(
                            sock,
                            id,
                            participant,
                            metadata.subject
