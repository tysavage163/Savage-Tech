...
module.exports = {
  name: 'eventsround',
  category: 'sports',
  description: 'Get sports data (eventsround)',
  async execute(sock, msg, args) {
    const query = args.join(' ');
    if (!query) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .eventsround <query>' });
    const sender = msg.pushName || 'User';
    const jid = msg.key.participant || msg.key.remoteJid;
    try {
      await sock.sendMessage(msg.key.remoteJid, { text: `🏆 Fetching eventsround...`, mentions: [jid] });
      const apiUrl = `https://apis.xwolf.space/api/sports/events/round?q=${encodeURIComponent(query)}`;
      const res = await axios.get(apiUrl, { httpsAgent: agent });
      const result = formatResult(res.data);
      const output = `🏅 *Sports: eventsround*\n👤 REQUESTED BY: @${sender}\n🔍 Query: ${query}\n\n${result}\n\n┍━━━━━━━━━━━━━━━╼
┃ 🚀 SΛVΛGΞ-TΞCH OS
┕━━━━━━━━━━━━━━━╼`;
