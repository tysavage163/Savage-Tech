...
module.exports = {
  ...
  async execute(sock, msg, args) {
    const url = args[0];
    if (!url) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .tiktok <URL>' });
    ...
    try {
      const apiUrl = `https://apis.xwolf.space/download/tiktok?url=${encodeURIComponent(url)}`;
      ...
      const isVideo = 'tiktok'.includes('video') || 'tiktok'.includes('mp4') || 'tiktok' === 'tiktok' || 'tiktok' === 'instagram' || 'tiktok' === 'facebook' || 'tiktok' === 'twitter' || 'tiktok' === 'snapchat';
      const isAudio = 'tiktok'.includes('mp3') || 'tiktok'.includes('audio');
      const isText = 'media' === 'text';

      if (isText) {
        // Send as text (info or search results)
        let text = `📁 *Download Info (tiktok)*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE\n\n`;
