...
module.exports = {
  name: 'ytsearch',
  category: 'download',
  description: 'Download from ytsearch',
  async execute(sock, msg, args) {
    const url = args[0];
    if (!url) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .ytsearch <URL>' });
    ...
    try {
      const apiUrl = `https://apis.xwolf.space/download/youtube/search?url=${encodeURIComponent(url)}`;
      ...
      const isVideo = 'ytsearch'.includes('video') || 'ytsearch'.includes('mp4') || 'ytsearch' === 'tiktok' || 'ytsearch' === 'instagram' || 'ytsearch' === 'facebook' || 'ytsearch' === 'twitter' || 'ytsearch' === 'snapchat';
      const isAudio = 'ytsearch'.includes('mp3') || 'ytsearch'.includes('audio');
      const isText = 'text' === 'text';

      if (isText) {
        // Send as text (info or search results)
        let text = `📁 *Download Info (ytsearch)*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE\n\n`;
      ...
      const caption = `📥 *Download: ytsearch*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
      if (isVideo) {
