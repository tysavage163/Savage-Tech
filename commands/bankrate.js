...
module.exports = {
  ...
  description: 'Get bankrate data',
  async execute(sock, msg, args) {
    const param = args[0];
    if (!param && 'bankrate' !== 'market' && 'bankrate' !== 'gold' && 'bankrate' !== 'news') {
      return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .bankrate <parameter>' });
    }
    ...
    try {
      let apiUrl = `https://apis.xwolf.space/api/economy/bank-rate`;
      let paramLabel = '';
      ...
      if ('bankrate' === 'forex') {
        ...
        // market, gold, news
        apiUrl = `https://apis.xwolf.space/api/economy/bank-rate`;
      }
      ...
      let output = `📊 *ECONOMIC DATA: bankrate*`;
