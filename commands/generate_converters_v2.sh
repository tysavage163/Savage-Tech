...
declare -A converters=(
  ["img-to-sticker"]="img-to-sticker"
  ["sticker-to-img"]="sticker-to-img"
  ["video-to-sticker"]="video-to-sticker"
  ["sticker-to-video"]="sticker-to-video"
  ["video-to-gif"]="video-to-gif"
  ["gif-to-video"]="gif-to-video"
)

for cmd in "${!converters[@]}"; do
  endpoint="${converters[$cmd]}"
  cat > "${cmd}.js" << 'ENDJS'
...
module.exports = {
  name: 'CMD_NAME',
  category: 'tools',
  description: 'Convert media using CMD_NAME (reply to image/video/sticker OR provide URL)',
  async execute(sock, msg, args) {
    ...
    if (!mediaBuffer && !providedUrl) {
      return sock.sendMessage(msg.key.remoteJid, { 
        text: '❓ Usage: .CMD_NAME\n   - Reply to an image/video/sticker\n   - Or provide a direct URL: .CMD_NAME https://example.com/media.jpg',
        mentions: [jid]
      });
    }
    ...
    try {
      ...
      if (mediaBuffer) {
        ...
        apiUrl = `https://apis.xwolf.space/api/converter/ENDPOINT?url=${encodeURIComponent(dataUrl)}`;
      } else {
        apiUrl = `https://apis.xwolf.space/api/converter/ENDPOINT?url=${encodeURIComponent(providedUrl)}`;
      }
      ...
      const caption = `✅ *Converted via CMD_NAME*\n👤 REQUESTED BY: @${sender}\n🚀 POWERED BY SAVAGE-CORE`;
...
ENDJS
  ...
  echo "✅ Created ${cmd}.js (supports reply & URL)"
done
echo "All converter commands regenerated with reply support."
