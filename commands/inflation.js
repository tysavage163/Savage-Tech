...
module.exports = {
  ...
  async execute(sock, msg, args) {
    const param = args[0];
    if (!param && 'inflation' !== 'market' && 'inflation' !== 'gold' && 'inflation' !== 'news') {
      return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .inflation <parameter>' });
    }
    ...
    try {
      let apiUrl = `https://apis.xwolf.space/api/economy/inflation`;
      let paramLabel = '';
      ...
      if ('inflation' === 'forex') {
        ...
        // market, gold, news
        apiUrl = `https://apis.xwolf.space/api/economy/inflation`;
      }
      ...
      let output = `📊 *ECONOMIC DATA: inflation*`;
