...
module.exports = {
  name: 'chrome',
  category: 'Ephoto',
  description: 'Generate chrome text effect',
  async execute(sock, msg, args) {
    const text = args.join(' ');
    if (!text) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .chrome <text>' });
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mentions = [senderJid];
    try {
      const apiUrl = `https://apis.xwolf.space/api/textpro/chrome?text=${encodeURIComponent(text)}`;
      ...
      const caption = `🎨 *Text Effect: chrome*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
