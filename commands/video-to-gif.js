...
module.exports = {
  name: 'video-to-gif',
  category: 'tools',
  description: 'Convert media using video-to-gif (reply to image/video/sticker OR provide URL)',
  async execute(sock, msg, args) {
    ...
    if (!mediaBuffer && !providedUrl) {
      return sock.sendMessage(msg.key.remoteJid, { 
        text: '❓ Usage: .video-to-gif\n   - Reply to an image/video/sticker\n   - Or provide a direct URL: .video-to-gif https://example.com/media.jpg',
        mentions: [jid]
      });
    }
    ...
    try {
      ...
      if (mediaBuffer) {
        ...
        apiUrl = `https://apis.xwolf.space/api/converter/video-to-gif?url=${encodeURIComponent(dataUrl)}`;
      } else {
        apiUrl = `https://apis.xwolf.space/api/converter/video-to-gif?url=${encodeURIComponent(providedUrl)}`;
      }
      ...
      const caption = `✅ *Converted via video-to-gif*\n👤 REQUESTED BY: @${sender}\n🚀 POWERED BY SAVAGE-CORE`;
