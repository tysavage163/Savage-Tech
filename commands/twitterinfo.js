...
module.exports = {
  name: 'twitterinfo',
  category: 'download',
  description: 'Download from twitterinfo',
  async execute(sock, msg, args) {
    const url = args[0];
    if (!url) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .twitterinfo <URL>' });
    ...
    try {
      const apiUrl = `https://apis.xwolf.space/download/twitter/info?url=${encodeURIComponent(url)}`;
      ...
      const isVideo = 'twitterinfo'.includes('video') || 'twitterinfo'.includes('mp4') || 'twitterinfo' === 'tiktok' || 'twitterinfo' === 'instagram' || 'twitterinfo' === 'facebook' || 'twitterinfo' === 'twitter' || 'twitterinfo' === 'snapchat';
      const isAudio = 'twitterinfo'.includes('mp3') || 'twitterinfo'.includes('audio');
      const isText = 'text' === 'text';

      if (isText) {
        // Send as text (info or search results)
        let text = `📁 *Download Info (twitterinfo)*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE\n\n`;
      ...
      const caption = `📥 *Download: twitterinfo*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
      if (isVideo) {
      ...
      console.error('twitterinfo error:', err);
