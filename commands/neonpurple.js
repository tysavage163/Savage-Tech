...
module.exports = {
  ...
  async execute(sock, msg, args) {
    ...
    if (!text) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .neonpurple <text>' });
    const senderName = msg.pushName || 'User';
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const mentions = [senderJid];
    try {
      const apiUrl = `https://apis.xwolf.space/api/textpro/neon-purple?text=${encodeURIComponent(text)}`;
