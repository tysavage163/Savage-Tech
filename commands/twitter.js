...
module.exports = {
  ...
  async execute(sock, msg, args) {
    const url = args[0];
    if (!url) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .twitter <URL>' });
    ...
    try {
      const apiUrl = `https://apis.xwolf.space/download/twitter?url=${encodeURIComponent(url)}`;
      ...
      const isVideo = 'twitter'.includes('video') || 'twitter'.includes('mp4') || 'twitter' === 'tiktok' || 'twitter' === 'instagram' || 'twitter' === 'facebook' || 'twitter' === 'twitter' || 'twitter' === 'snapchat';
      const isAudio = 'twitter'.includes('mp3') || 'twitter'.includes('audio');
      const isText = 'media' === 'text';

      if (isText) {
        // Send as text (info or search results)
        let text = `📁 *Download Info (twitter)*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE\n\n`;
      ...
      const caption = `📥 *Download: twitter*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
      if (isVideo) {
