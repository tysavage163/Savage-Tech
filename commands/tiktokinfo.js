...
module.exports = {
  name: 'tiktokinfo',
  category: 'download',
  description: 'Download from tiktokinfo',
  async execute(sock, msg, args) {
    const url = args[0];
    if (!url) return sock.sendMessage(msg.key.remoteJid, { text: '❓ Usage: .tiktokinfo <URL>' });
    ...
    try {
      const apiUrl = `https://apis.xwolf.space/download/tiktok/info?url=${encodeURIComponent(url)}`;
      ...
      const isVideo = 'tiktokinfo'.includes('video') || 'tiktokinfo'.includes('mp4') || 'tiktokinfo' === 'tiktok' || 'tiktokinfo' === 'instagram' || 'tiktokinfo' === 'facebook' || 'tiktokinfo' === 'twitter' || 'tiktokinfo' === 'snapchat';
      const isAudio = 'tiktokinfo'.includes('mp3') || 'tiktokinfo'.includes('audio');
      const isText = 'text' === 'text';

      if (isText) {
        // Send as text (info or search results)
        let text = `📁 *Download Info (tiktokinfo)*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE\n\n`;
      ...
      const caption = `📥 *Download: tiktokinfo*\n👤 REQUESTED BY: @${senderName}\n🚀 POWERED BY SAVAGE-CORE`;
      if (isVideo) {
      ...
      console.error('tiktokinfo error:', err);
